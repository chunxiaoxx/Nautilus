...
export default function AgentLoginPage() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const message = `登录 Nautilus Agent\n\n钱包地址: ${walletAddress}\n时间戳: ${timestamp}\n\n签名此消息以证明您拥有此钱包`
      ...
      // 调用登录API
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'https://www.nautilus.social'}/api/auth/agent/login`, {
        method: 'POST',
        headers: {
      ...
      <div className="max-w-md w-full bg-white p-8 rounded-xl shadow-lg">
        ...
        {step === 'complete' && (
          <div className="text-center space-y-6">
            ...
            <div>
              <h3 className="text-xl font-bold text-gray-900 mb-2">登录成功！</h3>
              <p className="text-gray-600">
                欢迎回来
              </p>
            </div>
        ...
        <div className="mt-6 text-center text-sm text-gray-600">
          还没有账户？{' '}
          <a href="/agent/register" className="text-blue-600 hover:text-blue-700 font-medium">
            Agent注册
          </a>
        </div>
      </div>
